...
int main(int argc, char* argv[]) {
	...
	for (i = 0; i < numQueries; i++) {
		...
		/*  Join 4: ?Y P2 ?x >< ?x P1 O1 */
		case 41:
			results = _ppoDerecha(treps, queriesDat[1][i], queriesDat[4][i],
					queriesDat[5][i] - 1,POROBJETO,PORSUJETO);
			break;
		case 42:
		...
		case 43:
			results = _ppoIndependiente(treps, queriesDat[1][i],
					queriesDat[4][i], queriesDat[5][i] - 1,POROBJETO,PORSUJETO);
			break;

		/*  Join 52: ?Y ?P2 ?x >< ?x P1 O1 */
		case 501:
			results = __poDerecha(treps, numPredicados, *indiceSP,*indiceOP,
								queriesDat[0][i]-1, queriesDat[1][i],POROBJETO,PORSUJETO);
			break;
		case 502:
		...
		/*  Join 5: ?Y P2 ?x >< ?x ?P1 O1 */
		case 511:
			results = _p_oDerecha(treps, numPredicados, *indiceSP,*indiceOP,
					queriesDat[1][i], queriesDat[5][i] - 1, 1,POROBJETO,PORSUJETO);
			break;
		case 512:
			results = _p_oDerecha(treps, numPredicados, *indiceSP, *indiceOP,
					queriesDat[1][i], queriesDat[5][i] - 1, 0,POROBJETO,PORSUJETO);
			break;
		case 521:
		...
		case 611:
			results = ___oDerecha(treps, numPredicados, *indiceSP, *indiceOP,
					queriesDat[5][i] - 1, 1,POROBJETO,PORSUJETO);
			break;
		case 612:
			results = ___oDerecha(treps, numPredicados, *indiceSP, *indiceOP,
					queriesDat[5][i] - 1, 0,POROBJETO,PORSUJETO);
			break;
