 ...
 *
 */
public class JNITriplesFullIterator implements IteratorTripleID {

	private int currentPredicate = -1;
	private int nPredicates;
	private JNITriples jnitriples;
	private JNITriplesIterator currentIterator;
	
	JNITriplesFullIterator(JNITriples triples, int npreds) {
		this.jnitriples = triples;
	...
	private void initializeNextPredicate() {
		...
		if (currentPredicate < nPredicates) {
			this.currentIterator = (JNITriplesIterator) jnitriples.search(new TripleID(0, currentPredicate+1, 0));
		}
	}
