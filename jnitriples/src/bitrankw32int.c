...
bitRankW32Int * createBitRankW32Int( uint *bitarray, uint _n, char owner, uint _factor) {
  bitRankW32Int * br =(bitRankW32Int *) mymalloc(sizeof(struct sbitRankW32Int));
  br->data=bitarray;
  br->owner = owner;
  br->n=_n;
...
void buildRank(bitRankW32Int * br) {
  ...
  br->Rs = (uint *) mymalloc(sizeof(uint)*(num_sblock+1));   // +1 pues sumo la pos cero
...
int load(bitRankW32Int * br, FILE *f) {
  ...
  //  integers = (n+1)/W;
  br->integers = n/W;
  br->data= (uint *) mymalloc(sizeof( uint) *(n/W+1));
  if (!br->data) return 1;
  if (fread (br->data,sizeof(uint),br->n/W+1,f) != n/W+1) return 25;
  br->owner = 1;
  br->Rs=(uint*)mymalloc(sizeof(uint)*(n/s+1));
  if (!br->Rs) return 1;
