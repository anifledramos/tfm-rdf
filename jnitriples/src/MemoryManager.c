...
of memory is needed (a unique call to mymalloc). It is also responsible of freeing memory.  
 ------------------------------------------------------------------------*/
...
MemoryManager createMemoryManager(void) {
	MemoryManager mm;
	mm = (MemoryManager) mymalloc (sizeof(struct sMem));
	mm->currentBlock=0;
...
void createNewMemoryBlock (MemoryManager mm) {
	mm->BLOCKS[mm->currentBlock] = (byte *) mymalloc (LARGE_BLOCK_SIZE);
