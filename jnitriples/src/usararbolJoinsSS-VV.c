...
int main(int argc, char* argv[]) {
	...
	for (i = 0; i < numQueries; i++) {
		switch (tipoQuery) {
			...
			/*  Join 7: ?x P2 ?Y >< ?x P1 ?Z */
			case 71:
				results = _pp_Izquierda(treps, queriesDat[1][i], queriesDat[4][i],PORSUJETO,PORSUJETO);
				break;

			case 72:
				results = _pp_Derecha(treps, queriesDat[1][i], queriesDat[4][i],PORSUJETO,PORSUJETO);
				break;

			case 73:
				results = _pp_Interactiva(treps, queriesDat[1][i],
						queriesDat[4][i],PORSUJETO,PORSUJETO);
				break;
			...
			/*  Join 8: ?Y P2 ?x >< ?x ?P ?Z */
			case 811:
				results = _p__Izquierda(treps, queriesDat[1][i], numPredicados,*indiceSP,*indiceOP,1,PORSUJETO,PORSUJETO);
				break;
			case 812:
				results = _p__Izquierda(treps, queriesDat[1][i], numPredicados,*indiceSP,*indiceOP,0,PORSUJETO,PORSUJETO);
				break;
			case 82:
				results = _p__Interactiva(treps, queriesDat[1][i],numPredicados, PORSUJETO,PORSUJETO);
				break;
